...
const plans = [
  {
    name: "Обычный",
    price: "от 250 ₽",
    features: ["Доставка в течение дня", "По всей Москве", "Документы и небольшие посылки", "Отслеживание статуса"],
  },
  {
    name: "Быстрый",
    price: "от 500 ₽",
  ...
  return (
    <section id="pricing" className="py-12 sm:py-20">
      <div className="container mx-auto">
        <h2 className="text-2xl sm:text-3xl font-bold text-center mb-8 sm:mb-12">Тарифы</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-8">
          {plans.map((plan, index) => (
            ...
            >
              <h3 className="text-xl sm:text-2xl font-bold mb-2 sm:mb-4">{plan.name}</h3>
              <p className="text-3xl sm:text-4xl font-bold mb-4 sm:mb-6">{plan.price}</p>
              ...
              <Button className="w-full hover:opacity-90 text-sm sm:text-base">Заказать</Button>
            </div>
          ))}
        </div>
        <p className="text-center mt-6 sm:mt-8 text-xs sm:text-sm text-muted-foreground">
          * Окончательная стоимость зависит от расстояния, веса и габаритов отправления
        </p>
      </div>
    </section>
  )
